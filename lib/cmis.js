...
(function (root, factory) {
  ...
  lib.createSession = function (url) {
    ...
    session.createDocument = function (parentId, content, input, mimeType, versioningState, policies, addACEs, removeACEs, options) {
      ...
      var url = _fixUnicodeUrl(properties);
      return _postMultipart(url,
    ...
    function CmisRequest(req, text) {
      ...
      /**
       * get the request for Progress or Cancel upload.
       * @returns {*}
       */
    ...
    var _proxyUrl = null;
    if (isNode && process.env.http_proxy) {
    ...
    var _fixUnicodeUrl = function(properties) {
      var key = 0, url;
      ...
      if(session.defaultRepository.rootFolderUrl.indexOf('?') > -1){
        url = session.defaultRepository.rootFolderUrl+"&propertyValue["+key+"]="+encodeURIComponent(properties['cmis:name']);
      }else{
        url = session.defaultRepository.rootFolderUrl+"?propertyValue["+key+"]="+encodeURIComponent(properties['cmis:name']);
      }
      return url
